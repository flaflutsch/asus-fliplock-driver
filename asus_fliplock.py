...
from libevdev import EV_SW, Device
            ...
            for line in lines:
                ...
                if wmi_hotkeys_detected == 1:
                    if "H: " in line:
                        ...
                        log.debug(
                            "Set wmi hotkeys id %s from %s", wmi_hotkeys, line.strip()
                        )
                        break
...
if switches is None and wmi_hotkeys is not None:
    for e in d_t_wmi_hotkeys.events():
        ...
        # Pressed and released event is called during one! flipping simultaneously
